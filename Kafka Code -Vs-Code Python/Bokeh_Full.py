...
# data_table_Raw = DataTable(source=source_Table_Raw, columns=columns, width=400, height=280,scroll_to_selection=False)
...
def Update_Prediction():
    ...
    prediction=[result_Prediction['prediction']], label=[result_Prediction['label']])
        ...
        # new_Data_Prediction=dict(x=[datetime.now()],y=[result_Prediction['user_id']])
    source_Prediction.stream(new_Data_Prediction,rollover=200)

        # source_Table_Raw.data=new_Data_TAble_Raw
  ...
  # the besr way to apply this algorithim is to work in this line 
def update_Raw():
    for message in consumer_Raw:
        result_raw= json.loads(message.value.decode('utf-8'))    
        break
    ...
    if result_raw['xvalue'] != NULL :   
        ...
        # source_Table_Raw.data=new_Data_Table_Raw
        # source_Table_Raw.stream(new_Data_TAble_Raw,rollover=200)
        # cccallback = CustomJS(args=dict(source=source_Table_Raw), code="""
        # last_row_idx = source.get_length();
        # source.selected.indices = [last_row_idx-1];""")
        # source_Table_Raw.js_on_change('data',cccallback)
...
lay_out =layout([p_Prediction, Table_Column],
                  [sliders,p_Raw])
# lay_out =layout([[p_Prediction],[p_Raw],[sliderx],[slidery],[sliderz]])
